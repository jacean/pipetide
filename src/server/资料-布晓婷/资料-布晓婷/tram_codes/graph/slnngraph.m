...
if ~isempty(dists)
    if isempty(opts.tfunctor)
        vals = dists;
         ...
         % 在slevalfunctor里面用feval函数求解，就是用函数internal_compaffinity(dists, nnidx, tralab, KK)求得vals
        vals = slevalfunctor(opts.tfunctor, dists, nnidx, tralab, KK);
    end
else
    vals = [];
end
